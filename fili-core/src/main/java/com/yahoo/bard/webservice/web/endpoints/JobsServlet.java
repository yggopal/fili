...
public class JobsServlet extends EndpointServlet {
    ...
    protected ResponseContext addPaginationInfoToResponseContext(
        ...
        LinkedHashMap<String, URI> bodyLinks = Arrays.stream(PaginationLink.values())
                ...
                .filter(pair -> pair.getRight().isEmpty().toBlocking().single())
                .map(pair -> Utils.withRight(pair, pair.getRight().toBlocking().single()))
        ...
        LinkedHashMap<String, URI> bodyLinkes = new LinkedHashMap<>();
        Arrays.stream(PaginationLink.values()).forEach(link ->
               link.getPage(pages)
                    ...
                    .subscribe(uri -> responseContext.put(ResponseContextKeys.PAGINATION_LINKS_CONTEXT_KEY.getName()))
        )
